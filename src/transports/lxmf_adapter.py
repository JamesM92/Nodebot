...
class LXMFAdapter:
    ...
    def __init__(self, storage_path, engine):
        ...
        local_only_raw = self._config.get(
            "logging", "announce_log_local_only", fallback="true"
        ).strip().lower()
        self._announce_local_only = local_only_raw in ("true", "1", "yes")
    ...
    @staticmethod
    def _is_local_path(destination_hash):
        """Return True if the last-known RNS path to destination_hash came via a
        local interface (LoRa/serial/auto) rather than TCP or UDP.

        RNS.Transport.destination_table entries are:
          [timestamp, hops, random_blob, announce_packet, receiving_interface]
        Falls back to True (log it) if the path is unknown or the table is
        inaccessible — better to over-log than silently drop local nodes.
        """
        try:
            ...
            return "TCP" not in iface_type and "UDP" not in iface_type
        except Exception:
            return True
    ...
    def is_connected(self):
        return self.running and self.router is not None
