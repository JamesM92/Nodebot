...
if [ ! -f "$CONFIG_INI" ]; then
    ...
    # Generate a 2-hex-char suffix for a unique default bot name.
    # Priority: last byte of primary MAC → last 2 chars of /etc/machine-id → random.
    _SUFFIX=$(ip link show 2>/dev/null \
        | awk '/ether/{print $2}' | grep -v '^00:00:00' | head -1 \
        | awk -F: '{print toupper($6)}')
    if [ -z "$_SUFFIX" ]; then
        _SUFFIX=$(cat /etc/machine-id 2>/dev/null | tr -d '[:space:]' | tail -c 2 | tr '[:lower:]' '[:upper:]')
    fi
    if [ -z "$_SUFFIX" ]; then
        _SUFFIX=$("$VENV_PYTHON" -c 'import secrets; print(secrets.token_hex(1).upper())')
    fi
    _SUFFIX="${_SUFFIX:-00}"
    sed -i "s/^name = NodeBot$/name = NodeBot-${_SUFFIX}/" "$CONFIG_INI"
    echo "  Created config.ini from config.example (bot name: NodeBot-${_SUFFIX})"
fi
