#!/usr/bin/python3
# NodeBot NomadNet node map page — deployed by install_lxmf.sh
...
def _map_age():
    """Return seconds since map was generated, or None if not ready."""
...
def _start_generation():
    ...
    with open(LOCKFILE, "w") as f:
        f.write("")
    subprocess.Popen(
        [VENV_PYTHON, WORKER, announce_db, storage_path, LOCKFILE],
        close_fds=True,
...
# ── Render ────────────────────────────────────────────────────
print(f"`!`F4af{bot_name}`f — Node Map`f")
print("-")
...
if fresh:
    ...
    print("")
    print("`F4af`[  ⬇  map.png`:/file/nodebot/map.png`]`f")
    print("")
    print("`F888Map will auto-regenerate after 5 minutes on next visit.`f")

elif _is_generating():
    print("`F888Map generation is in progress…`f")
    print("")
    print("`Faaa  Usually takes 3–10 seconds. Refresh to check.`f")
    print("")
    print("`Fbbf`[  ↻  Refresh`:/page/nodebot/map.mu`]`f")

else:
    _start_generation()
    print("`F888Map generation started in the background.`f")
    print("")
    print("`Faaa  Refresh this page in ~10 seconds to get the download link.`f")
    if age is None:
        print("`Faaa  (First run also fetches state boundary data ~500 KB)`f")
    print("")
    print("`Fbbf`[  ↻  Refresh`:/page/nodebot/map.mu`]`f")
