...
# that must be baked in at install time.
cat > "$NODE_PAGE" << 'NODEBOT_PAGE_EOF'
#!/usr/bin/python3
# NodeBot NomadNet info page — generated by install_lxmf.sh
import os
import re
import glob
import json
...
# Patch in the baked-in project directory path
sed -i "s|__PROJECT_DIR__|$PROJECT_DIR|g" "$NODE_PAGE"
chmod +x "$NODE_PAGE"
